...
- The addition operation is clearly an operation on $\mathbb{Z}$, as it always returns another integer ✔️
- Addition is associative ✔️
- The identity element is zero ✔️
- Each element $a\in\mathbb{Z}$ has an inverse, namely $-a$ ✔️
...
- The $+$-operation is associative ✔️
- There is a zero element: $0 + a = a + 0 =a$ ✔️
- Every element has an inverse ✔️ 
...
\end{array}
$$
- The zero element is $(0, 0)$ ✔️
- For every element, there is an inverse, for example $(2, 2) + (1, 2) = (0, 0)$ ✔️
...
- Matrix multiplication is associative ✔️
- Obvisouly, $GL_n(\mathbb{R})$ contains a zero element ✔️
- As the determinant is non-zero by definition, every matrix has an inverse matrix ✔️

### 🏷️ Example: $n$-th roots of unity

If we define $\mu_n = \{z\in\mathbb{C}\ |\ z^n=1 \}$ with multiplication, $(\mu_n, *)$ forms a group.

-  $\forall z_1, z_2 \in \mu_n: (z_1z_2)^n = z_1^nz_2^n =1$ ✔️
- Multiplication is associative ✔️
- There is one identiy element, namely $1^n = 1$ ✔️
- Every element has an inverse, as $1 / z^n$ is also in the group ✔️
